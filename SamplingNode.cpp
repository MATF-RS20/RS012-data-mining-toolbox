...
// Getter
double SamplingNode::SampleSizeRatio() const { return sampleSizeRatio; }
...
void SamplingNode::run() {
  ...
  // Making a vector with a field for every tuple that indicates whether the
  // tuple goes into the sample
  unsigned long rowSize = InputDataTable()->DataMatrix().n_rows;
  std::vector<bool> sample(rowSize);

  // Setting n (sample size) elements of vector to true indicating that sample
  // size elements will be taken
  // for sample. Setting the rest of elements to false.
  auto sampleSize = static_cast<unsigned long>(static_cast<double>(rowSize) *
                                               SampleSizeRatio());
  ...
  // Making new data matrix by sampling from the current one using the vector
  // for sampling
