...
PartitionParametersDialog::PartitionParametersDialog(PartitionNode *cvor,
                                                     QWidget *parent)
    : QDialog(parent), ui(new Ui::PartitionParametersDialog), prtCvor(cvor) {
  ui->setupUi(this);
}

PartitionParametersDialog::~PartitionParametersDialog() { delete ui; }
...
void PartitionParametersDialog::on_pushButton_clicked() {
  ...
  if (ok == false) {
    QMessageBox::information(
        this, "Greska!", "Pogresan Unos! Unestie broj u intervalu [0.0, 1.0]");
    return;
  }
  ...
  if (unos_d < 0.0 || unos_d > 1.0) {
    QMessageBox::information(this, "Greska!",
                             "Pogresan Unos! Broj nije u intervalu [0.0, 1.0]");
    return;
  }
