...
// Node is an abstract class that represents all the nodes in GUI. It has an
// abstract function run, that is specific for every subclass since every node
// that we will have in GUI has its own purpose. Since stream in GUI consists of
// nodes that are connected and the output of one node (algorithm) is input for
// the next, every object of class Node will have atributes inputDataTable and
// outputDataTable which will serve above mentioned purpose. Additionally, every
// node has an atribute that will represent its name.
class Node {
