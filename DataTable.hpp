...
private:
    ...
    //and the set of their possible values is remembered in in map categoricalValues, where key is the name of that column.
    std::map<std::string, std::set<std::string>> categoricalValues;
    
    //If there is partition of data, this vector has info for every tuple is it in training or in test set.
