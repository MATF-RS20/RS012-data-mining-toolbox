...
void FilterNode::run() {
  ...
  for (size_t i = 0; i != colNames.size(); i++) {
    ...
    // If the current column name exists in the set of column names that need to
    // be filtered
    if (columnNames.end() != columnNames.find(colNames[i])) {
      ...
      if (catCol.find(colNames[i]) != catCol.end()) {
        ...
        // Shed column with index i, numOfCol times (arma::mat shed_cols(i), all
        // indexes after i will decrement) i-numOfFiltered because of the above
        // reason (if we already sheded a column)
        while (numOfCol > 0) {

          matrix.shed_col(i + catColSum - numOfFiltered);
          numOfCol--;
        }
        // Otherwise, just shed i+catColSum (add the number of categorical
        // binarized columns that we didnt filter)
      } else {
      ...
      // Otherwise if column name is a key in the map categoricalValues,
      // increase catColSum by the number of columns we skipped
      if (catCol.find(colNames[i]) != catCol.end()) {
