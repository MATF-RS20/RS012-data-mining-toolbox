...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), broj_cvorova(0)
...
void MainWindow::on_ConnectButton_clicked() {
  ...
  // u zavisnosti od oznake cvora postavjaju se vrednosti promenljivih cvor1 i
  // cvor2 videti klasu SceneNode za detaljnije objasnjenje oznake
  ...
  for(std::pair<Node*, Node*> e : mapaCvorova){
      ...
      if(e.second == nullptr)
          ...
          std::cout << e.first->NodeName() << ":" << e.second->NodeName() <<
  std::endl;
  }
  */
}
...
void MainWindow::on_DisconnectButton_clicked() {
  ...
  for (auto i : sviElementi) {
    if ((nekaLinija = dynamic_cast<ConnectionLine *>(i))) {
      ...
      if (nekaLinija->getPocetniCvor() == cvor1 &&
          nekaLinija->getKrajnjiCvor() == cvor2) {
...
QString MainWindow::generisiID(const QString &nazivAlgoritma) {
  ...
  // kao ID njihovOG odogovarajucEG cvora na sceni

  // pri generisanju jedinstvenog ID-a za cvorove brine se i o ukupnom broju
  // cvorova f-ja uvecava ukupan broj cvorova
...
void MainWindow::on_SetParameters_clicked() {
  ...
  // pozivanje odgovarajuce fje za postavljanje parametara u zavisnosti od tipa
  // odabranog cvora svaka fja otvara poseban prozor za unos podataka
...
// u nastavku slede fje za otvaranje prozora u kojima korisnik moze postaviti
// parametre za odabrane cvorova
